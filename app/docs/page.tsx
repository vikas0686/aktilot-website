...
const openaiQuickStart = `git clone ${siteConfig.githubUrl}.git
cd Aktilot
...
const ollamaQuickStart = `git clone ${siteConfig.githubUrl}.git
cd Aktilot
...
const services = [
  ...
  { name: "Ollama", url: "http://localhost:11434", purpose: "Local LLM server (only with --profile ollama)" },
];

const envVars = [
  { name: "OPENAI_API_KEY", required: "Conditional", desc: "Your OpenAI API key (if using OpenAI)" },
  ...
  { name: "LLM_PROVIDER", required: "No", desc: "Chat provider (default: openai). Currently supported: openai, ollama" },
  { name: "CHAT_MODEL", required: "No", desc: "Chat model to use (default: gpt-4o-mini)" },
  { name: "EMBEDDING_PROVIDER", required: "No", desc: "Embedding provider (default: openai). Currently supported: openai, ollama" },
  { name: "EMBEDDING_MODEL", required: "No", desc: "Embedding model (default: text-embedding-3-small)" },
  { name: "OLLAMA_BASE_URL", required: "Conditional", desc: "Ollama server URL (default: http://localhost:11434)" },
          ...
          tabs={[
            {
              id: "openai",
              label: "OpenAI",
              code: openaiQuickStart,
              codeLabel: "Terminal",
            },
            {
              id: "ollama",
              label: "Ollama",
              code: ollamaQuickStart,
              codeLabel: "Terminal",
            },
          ]}
        />
